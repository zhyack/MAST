...
class Seq2SeqModel():
    def __init__(self, config):
        ...
        self.encoder_targets = tf.placeholder(dtype=tf.int32, shape=(None, self.batch_size), name='ebcoder_targets')
        self.encoder_targets_length = tf.placeholder(dtype=tf.int32, shape=(self.batch_size, ), name='encoder_targets_length')
        self.encoder_targets_mask = tf.placeholder(dtype=tf.float32, shape=(self.batch_size, None), name='encoder_targets_mask')
            ...
            with tf.variable_scope(tf.get_variable_scope(), reuse=True if ru else None):
                ...
                with tf.variable_scope(config['MODEL_PREFIX']+"DynamicDecoder") as scope:
                    ...
                    if config['IS_TRAIN']:
                        ...
                        self.decoder_baoutputs = modelInitDecoderForTrain(self.decoder_cell, self.decoder_inputs_embedded, self.decoder_inputs_length, initial_state, None)
                        self.decoder_baoutputs = tf.transpose(self.decoder_baoutputs, [1,0,2])
                        self.bacoder_cell = modelInitRNNCells(self.decoder_hidden_size, config['DECODER_LAYERS'], config['CELL'], config['INPUT_DROPOUT'], config['OUTPUT_DROPOUT'])
                        if config['ATTENTION_DECODER']:
                            self.bacoder_cell = modelInitAttentionDecoderCell(self.bacoder_cell, self.decoder_hidden_size, self.decoder_baoutputs, self.decoder_inputs_length, att_type=config['ATTENTION_MECHANISE'], wrapper_type='whole')
                        else:
                            self.bacoder_cell = modelInitRNNDecoderCell(self.bacoder_cell)
                        ba_initial_state = self.bacoder_cell.zero_state(batch_size=self.batch_size, dtype=tf.float32)
                        self.baoutput_projection_layer = layers_core.Dense(self.output_size_1, use_bias=False, name=config['MODEL_PREFIX']+'Op2')
                        self.back_outputs = modelInitDecoderForTrain(self.bacoder_cell, self.encoder_inputs_embedded_1, self.encoder_inputs_length, ba_initial_state, self.baoutput_projection_layer)
                    if config['USE_BS'] and not config['IS_TRAIN']:
                ...
                if config['IS_TRAIN']:
                    ...
                    if mode==2:
                        ...
                        outputs2use = self.back_outputs

                    self.train_loss_rl.append(seq2seq.sequence_loss(logits=self.back_outputs, targets=tf.transpose(self.encoder_targets, perm=[1,0]), weights=self.encoder_targets_mask))
                    if mode!=2:
    ...
    def make_feed(self, encoder_inputs, encoder_inputs_length, encoder_inputs_mask, encoder_targets, encoder_targets_length, encoder_targets_mask, decoder_inputs, decoder_inputs_length, decoder_inputs_mask, decoder_targets, decoder_targets_length, decoder_targets_mask):
        return {
            self.encoder_inputs: encoder_inputs,
            self.encoder_inputs_length: encoder_inputs_length,
            self.encoder_inputs_mask: encoder_inputs_mask,
            self.encoder_targets: encoder_targets,
            self.encoder_targets_length: encoder_targets_length,
            self.encoder_targets_mask: encoder_targets_mask,
    ...
    def train_on_batch(self, session, encoder_inputs, encoder_inputs_length, encoder_inputs_mask, encoder_targets, encoder_targets_length, encoder_targets_mask, decoder_inputs, decoder_inputs_length, decoder_inputs_mask, decoder_targets, decoder_targets_length, decoder_targets_mask, mode=0):
        train_feed = self.make_feed(encoder_inputs, encoder_inputs_length, encoder_inputs_mask, encoder_targets, encoder_targets_length, encoder_targets_mask, decoder_inputs, decoder_inputs_length, decoder_inputs_mask, decoder_targets, decoder_targets_length, decoder_targets_mask)
        ce_loss, rlloss = 0, 0
    ...
    def eval_on_batch(self, session, encoder_inputs, encoder_inputs_length, encoder_inputs_mask, decoder_inputs, decoder_inputs_length, decoder_inputs_mask, decoder_targets, decoder_targets_length, decoder_targets_mask):
        infer_feed = self.make_feed(encoder_inputs, encoder_inputs_length, encoder_inputs_mask, decoder_targets, decoder_targets_length, decoder_targets_mask, decoder_inputs, decoder_inputs_length, decoder_inputs_mask, decoder_targets, decoder_targets_length, decoder_targets_mask)
