...
def initSaver(vars_to_save, max_to_keep=3):
...
def modelInitPretrainedDecoder(cell, inputs, inputs_lengths, initial_state,  output_projection_layer):
    train_helper = tf.contrib.seq2seq.TrainingHelper(inputs, inputs_lengths, time_major=True)
    decoder=tf.contrib.seq2seq.BasicDecoder(cell, helper=train_helper, initial_state=initial_state, output_layer=output_projection_layer)
    outputs, states, sequence_lengths = tf.contrib.seq2seq.dynamic_decode(decoder, impute_finished=True, maximum_iterations=None)
    return decoder, outputs.rnn_output

def modelInitMultiDecodersForTrain(decoders, ma_policy, maps_g2l, word_embedding_matrixs, decode_func, policy_mode):
    outputs, states, sequence_lengths = decode_func(decoders, ma_policy, maps_g2l, word_embedding_matrixs, impute_finished=True, maximum_iterations=None, policy_mode=policy_mode)
    outputs, weights = outputs[0], outputs[1]
...
def modelInitMultiDecodersForGreedyInfer(decoders, max_len, word_embedding_matrixs, ma_policy, maps_g2l, decode_func):
    outputs, states, sequence_lengths = decode_func(decoders, ma_policy, maps_g2l, word_embedding_matrixs, impute_finished=False, maximum_iterations=max_len*2)
    outputs, weights = outputs[0], outputs[1]
    return outputs.sample_id
...
def modelInitMultiDecodersForBSInfer(cells, inputs, word_embedding_matrixs, beam_width, id_ends, max_len, initial_states, output_projection_layers, ma_policy, maps_g2l, decode_func):
    decoders = []
    for cell, word_embedding_matrix, id_end, initial_state, output_projection_layer in zip(cells, word_embedding_matrixs, id_ends, initial_states, output_projection_layers):
        decoder=tf.contrib.seq2seq.BeamSearchDecoder(cell, word_embedding_matrix, inputs, id_end, initial_state=initial_state, beam_width=beam_width, output_layer=output_projection_layer)
    outputs, states, sequence_lengths = decode_func(decoders, ma_policy, maps_g2l, word_embedding_matrixs, output_projection_layers, impute_finished=False, maximum_iterations=max_len*2)
    return tf.transpose(outputs.predicted_ids, [2,0,1])[0]
