...
CONFIG['MODELS_INPUT_VOCAB_SIZES'] = []
CONFIG['MODELS_OUTPUT_VOCAB_SIZES'] = []
CONFIG['IDS_END'] = []
for prefix in CONFIG['MODEL_PREFIX']:
    d, rd = loadDict('auto-train-cc/'+prefix+'/all.in.dict')
    CONFIG['MODELS_INPUT_VOCAB_SIZES'].append(len(rd))
    d, rd = loadDict('auto-train-cc/'+prefix+'/all.out.dict')
    CONFIG['MODELS_OUTPUT_VOCAB_SIZES'].append(len(rd))
    CONFIG['IDS_END'].append(d['<EOS>'])
...
train_buckets_raw = train_buckets_raw_parallel
dict_train_src = full_dict_src
dict_train_dst = full_dict_dst
...
g = tf.Graph()
tfconfig = tf.ConfigProto(gpu_options=tf.GPUOptions(per_process_gpu_memory_fraction=0.9))
    ...
    with tf.Session(config=tfconfig) as sess:
        ...
        for n_iter in range(CONFIG['GLOBAL_STEP']/CONFIG['MAX_STEPS_PER_ITER'], CONFIG['ITERS']):
            while True:
                ...
                model_inputs, len_inputs, inputs_mask = dataSeqs2NpSeqs(train_batch[0], dict_train_src, CONFIG['BUCKETS'][b][0])
                model_outputs, len_outputs, outputs_mask = dataSeqs2NpSeqs(train_batch[1], dict_train_dst, CONFIG['BUCKETS'][b][1])
                model_targets, len_targets, targets_mask = dataSeqs2NpSeqs(train_batch[1], dict_train_dst, CONFIG['BUCKETS'][b][1], bias=1)
                ...
                print('Train completed for Iter@%d, Step@%d: CE_Loss=%.6f LM_Loss=%.6f Loss=%.6f LR=%.8f'%(n_iter, CONFIG['GLOBAL_STEP'], batch_loss[0], batch_loss[1], batch_loss[0]+batch_loss[1], CONFIG['LR']))
                CONFIG['GLOBAL_STEP']+=1
                lr_decay_func()
