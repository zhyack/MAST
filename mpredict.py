...
parser.add_argument(
    ...
    type=str,
    default=None,
    help="The specified reference file to compute bleu.")
parser.add_argument(
    "-n",
    dest="model_index",
    type=int,
...
blank_output = ''
for i in range(CONFIG['MAX_OUT_LEN']*2):
    blank_output += '<UNK> '
test_raw = [ [x_test_raw[i].strip(), blank_output] for i in range(len(x_test_raw))]
...
with g.as_default():
    ...
    with tf.Session(config=tfconfig) as sess:
        ...
        for b in range(len(CONFIG['BUCKETS'])):
            ...
            for k in range((n_b+CONFIG['BATCH_SIZE']-1)/CONFIG['BATCH_SIZE']):
                ...
                for i in range(CONFIG['BATCH_SIZE']):
                    test_results[test_batch[i][0]] = dataLogits2Seq(predict_outputs[i], rev_dict_dst, calc_argmax=True)
        f_x = codecs.open(args.input_file,'r','UTF-8')
        ...
        if args.refer_file:
            command_s = 'python utils/multi_bleu.py -hyp %s -ref %s'%(args.refer_file, args.load_folder+fname)
            print("BLEU:")
